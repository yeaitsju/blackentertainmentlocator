import"./modulepreload-polyfill.c7c6310f.js";const m=()=>{document.querySelector("#app").innerHTML=`
  ...
  border: 1px solid #FFFFFF`,h=L.divIcon({className:"my-custom-pin",iconAnchor:[0,24],labelAnchor:[-6,0],popupAnchor:[0,-36],html:`<span style="${p}" />`});L.marker([o.coordinates.latitude,o.coordinates.longitude],{icon:h}).addTo(s).bindPopup(`
        <h3>${o.name}</h3>
        <img src="${o.image_url}" />
        <p>${o.display_phone}</p>
        <a href="${o.url}">More Info...</a>
    `)}};function u(e,t){let o=`https://www.apitutor.org/yelp/v3/businesses/search?location=${t}&categories=${e}&limit=50&term=black owned`;e=="museums"&&(o=`https://www.apitutor.org/yelp/v3/businesses/search?location=${t}&categories=${e}&limit=50`),console.log(o),fetch(o).then(i=>i.json()).then(i=>{i.businesses.length==0?alert("No BOB. Why?!?!"):(g(i),f(i))})}const g=e=>{console.log(e);const t=e.businesses.map(o=>(console.log(o.name,o.rating),v(o)));document.querySelector(".list").innerHTML=t.join("")};function v(e){let t="result";return c.includes(e.id)&&(t="result black-owned"),`
  <section class="${t}">
     <img src= "${e.image_url}"/>
     <div> 
          <h3>${e.name} </h3>
